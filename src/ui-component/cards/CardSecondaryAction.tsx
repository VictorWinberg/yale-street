...
const CardSecondaryAction = ({ link, title, icon }: CardSecondaryActionProps) => {
  const theme = useTheme();

  const LinkComponent: React.ElementType = forwardRef((props, ref: React.Ref<HTMLAnchorElement>) => (
    <Link ref={ref} {...props} href={link} target="_blank" />
  ));
  ...
  return (
    <Tooltip title={title || 'Reference'} placement="left">
      <ButtonBase disableRipple>
        {!icon && (
          <Avatar component={LinkComponent} aria-label="redirect pages" alt="MUI Logo" size="badge" outline>
        ...
        {icon && (
          <Avatar component={LinkComponent} size="badge" color="primary" outline aria-label="material-ui">
            {icon}
          </Avatar>
        )}
      </ButtonBase>
